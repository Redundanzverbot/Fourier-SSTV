...
void draw(){
  ...
  // FFT anzeigen
    ...
    // Zeilenweise anzeigen
    for (int y = 0; y < imgHeight[mode]; y++){
      
      float baseFreq = fftSpectrum[1 + y * (imgWidth[mode]+1)];
      
      for (int x = 0; x < imgWidth[mode]; x++){
        fill((fftSpectrum[2 + x + y * (imgWidth[mode]+1)] / baseFreq) * 255);
...
void showImage(){
  ...
  for (int y = 0; y < imgHeight[mode]; y++){
      ...
      float baseFreq = buttons[1].amplitudes[1 + y * (imgWidth[mode]+1)];
      
      for (int x = 0; x < imgWidth[mode]; x++){
        fill((buttons[1].amplitudes[2 + x + y * (imgWidth[mode]+1)] / baseFreq) * 255);
        rect(x * res, y * res, res, res);
      }
    }
